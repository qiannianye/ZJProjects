...
class UserAPI: HttpAPIManager {
    ...
    func thirdLogin(openId: String, unionId: String, platform: String, accessToken: String, clientId: String, expiresIn: String, appId: String) -> AnyAPIProducer {
        ...
        let confi = HttpRequestConfiguration(url: "/2.3/auth/token.json", method: .post, paraDic: dic, isToken: false)
        return producer(confi: confi)
    }
    ...
    func wxAuth(code: String){
        ...
        let confi = HttpRequestConfiguration(url: wxAuthUrl, method: .get, paraDic: Dictionary<String,Any>(), isToken: false)
    ...
    func userVipInfo(needInfo: String) -> AnyAPIProducer {
        ...
        let confi = HttpRequestConfiguration(url: "/2.3/vip/user/vipInfo.json", method: .post, paraDic: dic, isToken: true)
        return producer(confi: confi)
    }
    
    func userSignIn() -> AnyAPIProducer {
        let confi = HttpRequestConfiguration(url: "/2.3/user/checkIn.json", method: .post, paraDic: nil, isToken: true)
        return producer(confi: confi)
    }
}
